...
async function main() {
  ...
  const output = {
    ...
    Filter: "Work Arrangement exactly Remote plus an explicit non-US country, region, or worldwide location marker. Multi-region listings can overlap with the US report.",
    Counts: {
  ...
  const markdownPath = path.join(reportsDir, "international-remote-daily-report.md");
  const jsonPath = path.join(reportsDir, "international-remote-daily-report.json");
  ...
  console.log("Explicit international-remote daily report complete.");
